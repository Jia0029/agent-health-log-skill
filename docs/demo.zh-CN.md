...
这里的重点不只是“写入一行 CSV”。CSV 是后续复盘、趋势分析、可视化和本地 dashboard 的原材料。完整示例可以看 [效果展示](../examples/outcome-showcase.md)。
...
当本地积累了几天脱敏样例记录后，Agent 可以生成这样的复盘：
...
| 指标 | 本周 | 说明 |
| --- | ---: | --- |
| 硬拉最高工作组 | 80kg x 5 | 形成稳定基线 |
| 已记录蛋白质日均 | 92g/天 | 粗略估算 |
| 饮食记录覆盖率 | 5/7 天 | 有 2 天漏记 |
| 睡眠均值 | 6.6 小时 | 训练日偏低 |

## 下次复盘可以继续看

- 睡眠少的日子，训练表现是否更弱？
- 蛋白质记录是否足够稳定，可以用于趋势分析？
- 下一个重点追踪动作是什么？
```

也可以输出给图表或 dashboard 使用的数据：
...
这就是为什么项目同时保留结构化 CSV 和 `raw_text`：用户输入时足够轻量，后续又能用于复盘和可视化。
